...
def graph_plotter(plot_information, values_list, x_limits=("x_min", "x_max"),
    ...
    if isinstance(x_limits[0], (float, int)) and isinstance(x_limits[1], (float, int)):
        plt.xlim(x_limits)
    # Checks to see if plot limits have been set for the y values
    if isinstance(y_limits[0], (float, int)) and isinstance(y_limits[1], (float, int)):
        plt.ylim(y_limits)
...
def multiaxis_graph_plotter(plot_information_left, plot_information_right,
    ...
    # axis.
    if isinstance(y_limits_left[0], (float, int)) and isinstance(y_limits_left[1], (float, int)):
        axis_1.set_ylim(y_limits_left)
        # Checks to see if plot limits have been set for the y values on the
        # right axis.
    if isinstance(y_limits_right[0], (float, int)) and isinstance(y_limits_right[1], (float, int)):
